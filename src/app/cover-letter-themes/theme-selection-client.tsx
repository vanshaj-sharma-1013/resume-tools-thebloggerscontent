...
import { CoverLetterThemeMiniPreview } from "@/components/cover-letter-theme-mini-preview";
import { useCoverLetterDraft } from "@/hooks/use-cover-letter-draft";
import { saveCoverLetterDraftToStorage } from "@/lib/cover-letter-draft";
import type { CoverLetterThemeId } from "@/lib/cover-letter-themes";
import { COVER_LETTER_THEMES } from "@/lib/cover-letter-themes";

export function CoverLetterThemeSelectionClient() {
  ...
  const selectTheme = (id: CoverLetterThemeId) => {
    setError(null);
  ...
  const handleContinue = () => {
    ...
    router.push("/cover-letter-maker");
  };
  ...
  return (
    <div className="relative flex min-h-full flex-1 flex-col overflow-hidden bg-background">
      <BuilderBackground />
      <BuilderHeader
        eyebrow="flow.cover_letter"
        title="Choose cover letter design"
        subtitle="Four professional templates · same export quality as your resume"
      />

      <main className="relative z-10 mx-auto w-full max-w-6xl flex-1 px-4 py-10 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-3xl text-center">
          <p className="text-sm leading-relaxed text-foreground-muted">
            Select the design that matches your resume for a consistent brand.
            Your choices are saved locally.
          </p>
          <p className="mt-2 font-mono text-xs text-secondary">
            ~10 seconds · saves automatically
          </p>
        </div>
              ...
              return (
                <li key={theme.id}>
                  <button
                    ...
                    className={`group flex w-full flex-col rounded-2xl border p-4 text-left transition-all sm:p-5 ${isActive
                    ...
                    <div className="flex flex-1 flex-col gap-4 sm:flex-row sm:items-stretch">
                      ...
                      <div className="flex min-w-0 flex-1 flex-col">
                        <div className="flex flex-wrap items-start justify-between gap-2">
                          ...
                          <span
                            className={`shrink-0 rounded-full px-2 py-0.5 font-mono text-[10px] uppercase tracking-wide ring-1 ring-charcoal-border ${isActive
